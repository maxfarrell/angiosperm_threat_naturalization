...
### With diversification x clade age interaction



```{r rare_nat_vetted_interaction, echo=TRUE, eval=TRUE}

if (!file.exists("./vet_rare_naturalized_interaction_10k.rds")) {

  vet_rare_nat <- brm(rare | trials(vetted) ~ log10(Naturalised+1) + diversification*new_age +log10(species) +  (1|family) + (1|family_name), 
    dat=data_vetted, family=binomial(), 
    iter=10000, thin=4, cov_ranef = list(family = phy_cov_vetted),
    control=list(adapt_delta=0.95,max_treedepth=12), cores=4)
...
# names1 <- c("Number of naturalized species","Diversification x clade age","Number of species","SD brownian effects","SD family specific effects")
...
<!-- OLD CODE & ALTERNATIVE MODELS  -->


```{r rare_naturalized, echo=F, eval=F}
