...
Visualizatino of brms default priors (blue) compared to custom priors (yellow) inspired by McElreath's book Statistical Rethinking.

Priors for the slope parameters:

```{r prior_predict, echo=TRUE, eval=TRUE}
...
plot(density(prior2, adj=0.1), xlim=c(0,10), col="#E69F00", main="", xlab=expression(paste("inv_logit (",sigma,")")))
...
pos <- as.array(vet_rare_p2)
np <- nuts_params(vet_rare_p2)
png("./plots_tables/vet_rare_p2_pairs.png", width=30, height=24)
mcmc_pairs(pos, np=np, pars=rownames(to_print))
dev.off()
