...
# if (!file.exists("./phyo_mod_rare_invasive_4k.rds")) {

#   phylo_mod_rare_inv <- brm(rare | trials(species) ~ log10(Invasive+1) + log10(new_age) +log10(species) +  (1|family) + (1|family_name), 
#     dat=data, family=binomial(), 
#     iter=4000, cov_ranef = list(family = phy_cov),
#     control=list(adapt_delta=0.95,max_treedepth=12), cores=4)

#   saveRDS(phylo_mod_rare_inv, "./phyo_mod_rare_invasive_4k.rds")

# } else { phylo_mod_rare_inv <- readRDS("./phyo_mod_rare_invasive_4k.rds")}
...
# # # https://cran.r-project.org/web/packages/sjstats/vignettes/bayesian-statistics.html

# var_family <- posterior_samples(phylo_mod_rare_inv, pars = "sd_family__Intercept")
# var_family_name <- posterior_samples(phylo_mod_rare_inv, pars = "sd_family_name__Intercept")
# varProp_full <- (var_family^2)/(var_family^2 + var_family_name^2 + (pi^2)/3)
# plot(density(varProp_full))

# hyp <- paste("sd_family__Intercept^2 /", "(sd_family__Intercept^2 + sd_family_name__Intercept^2 + 3.289868) = 0")
# (hyp <- hypothesis(phylo_mod_rare_inv, hyp, class = NULL))
# plot(hyp)

# hyp <- paste("sd_family_name__Intercept^2 /", "(sd_family__Intercept^2 + sd_family_name__Intercept^2 + 3.289868) = 0")
# (hyp <- hypothesis(phylo_mod_rare_inv, hyp, class = NULL))


# sjstats::icc(phylo_mod_rare_inv)
# icc(phylo_mod_rare_inv, ppd=TRUE)

# icc(phylo_mod_rare_inv, ppd=TRUE, re.form=~(1|family), typical="median")

# icc(phylo_mod_rare_inv, posterior = TRUE) %>% print(prob = .95, digits = 3)
# icc(phylo_mod_rare_inv, posterior = TRUE, ppd=TRUE, typical="median") %>% print(prob = .95, digits = 3)

# icc(phylo_mod_rare_inv, posterior = TRUE, ppd=TRUE, typical="median")

# test <- icc(phylo_mod_rare_inv, posterior = TRUE)

# sum(get_re_var(phylo_mod_rare_inv))

# get_re_var(phylo_mod_rare_inv)
