...
# creating another family level variable
...
# var_full <- posterior_samples(rare_inv, pars = "sd_family__Intercept")
# varProp_full <- (var_full^2)/(var_full^2 + (pi^2)/3)
# hyp <- paste("sd_family__Intercept^2 /", "(sd_family__Intercept^2 + 3.289868) = 0")
# (hyp <- hypothesis(rare_inv, hyp, class = NULL))

# icc(rare_inv)
# icc(rare_inv, ppd=TRUE)

# icc(rare_inv, ppd=TRUE, re.form=~(1|family), typical="median")

# icc(rare_inv, posterior = TRUE) %>% print(prob = .95, digits = 3)
# icc(rare_inv, posterior = TRUE, ppd=TRUE, typical="median") %>% print(prob = .95, digits = 3)

# icc(rare_inv, posterior = TRUE, ppd=TRUE, typical="median")


# test <- icc(rare_inv, posterior = TRUE)

# sum(get_re_var(rare_inv))
...
if (!file.exists("./fit_rare_2k.rds")) {
  ...
  fit_rare <- brm(rare | trials(species) ~ woodiness + climate.sum + shybrids + log10(new_age) + log10(species) + hermaphroditic + dioec2 + monoec2 + asexual + (1 | family), 
    dat=data, family=binomial(), 
    iter=2000, cov_ranef = list(family = phy_cov),
...
if (!file.exists("./fit_nat_2k.rds")) {
  ...
  fit_nat <- brm(Naturalised | trials(species) ~ woodiness + climate.sum + shybrids + log10(new_age) + log10(species) + hermaphroditic + dioec2 + monoec2 + asexual + (1 | family), 
    dat=data, family=binomial(), 
    iter=2000, cov_ranef = list(family = phy_cov),
...
if (!file.exists("./fit_inv_2k.rds")) {
  ...
  fit_inv <- brm(Invasive | trials(species) ~ woodiness + climate.sum + shybrids + log10(new_age) + log10(species) + hermaphroditic + dioec2 + monoec2 + asexual + (1 | family), 
    dat=data, family=binomial(), 
    iter=2000, cov_ranef = list(family = phy_cov),
...
if (!file.exists("./fit_weed_2k.rds")) {
  ...
  fit_weed <- brm(Weed | trials(species) ~ woodiness + climate.sum + shybrids + log10(new_age) + log10(species) + hermaphroditic + dioec2 + monoec2 + asexual + (1 | family), 
    dat=data, family=binomial(), 
    iter=2000, cov_ranef = list(family = phy_cov),
...
if (!file.exists("./fit_ag_2k.rds")) {
  ...
  fit_ag <- brm(Agricultural.Weed | trials(species) ~ woodiness + climate.sum + shybrids + log10(new_age) + log10(species) + hermaphroditic + dioec2 + monoec2 + asexual + (1 | family), 
    dat=data, family=binomial(), 
    iter=2000, cov_ranef = list(family = phy_cov),
    control=list(adapt_delta=0.8, max_treedepth=12), cores=2)
...
if (!file.exists("./fit_env_2k.rds")) {
  ...
  fit_env <- brm(Environ | trials(species) ~ woodiness + climate.sum + shybrids + log10(new_age) + log10(species) + hermaphroditic + dioec2 + monoec2 + asexual + (1 | family), 
    dat=data, family=binomial(), 
    iter=2000, cov_ranef = list(family = phy_cov),
    control=list(adapt_delta=0.8, max_treedepth=12))
...
if (!file.exists("./fit_nox_2k.rds")) {
  ...
  fit_nox <- brm(Noxious | trials(species) ~ woodiness + climate.sum + shybrids + log10(new_age) + log10(species) + hermaphroditic + dioec2 + monoec2 + asexual + (1 | family), 
    dat=data, family=binomial(), 
    iter=2000, cov_ranef = list(family = phy_cov),
    control=list(adapt_delta=0.8, max_treedepth=12))
